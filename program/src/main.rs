...
fn main() {
    ...
    op_succinct_client_utils::block_on(async move {
        ...
        #[cfg(feature = "kroma")]
        {
            ...
            let l2_safe_header = cursor.l2_safe_head_header();
            ...
            let l1_origin = cursor.l2_safe_head().l1_origin;
    ...
    O: CommsClient + FlushableCache + FlushableCache + Send + Sync + Debug,
{
    loop {
        ...
        let _ = l2_provider.update_cache(header, &block, &boot.rollup_config).unwrap();

        // Get the pipeline origin and update the cursor.
        let origin = pipeline.origin().ok_or(PipelineError::MissingOrigin.crit())?;
        let l2_info =
        ...
        let tip_cursor = TipCursor::new(
            ...
            block_executor.compute_output_root().map_err(DriverError::Executor)?,
        );
